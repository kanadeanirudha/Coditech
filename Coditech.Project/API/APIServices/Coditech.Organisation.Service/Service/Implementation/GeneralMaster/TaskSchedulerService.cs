			...
			if (taskSchedulerData?.TaskSchedulerMasterId > 0)
			{
				...
				if (taskSchedulerModel.IsCronJob && taskSchedulerModel.IsEnabled)
				{
					...
					if (schedulerStatus)
					{
						...
						taskSchedulerData.ConfiguratorId = taskSchedulerModel.TaskSchedulerMasterId;
						_taskSchedulerRepository.Update(taskSchedulerData);
					}
					else
					{
						DeleteTaskScheduler(new ParameterModel() { Ids = taskSchedulerModel.TaskSchedulerMasterId.ToString() });
