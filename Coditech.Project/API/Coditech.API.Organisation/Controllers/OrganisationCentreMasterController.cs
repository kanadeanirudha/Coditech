...
namespace Coditech.API.Controllers
{
    public class OrganisationCentreMasterController : BaseController
    {
        ...
        public IActionResult CreateOrganisationCentre([FromBody] OrganisationCentreMasterModel model)
        {
            try
            {
                OrganisationCentreMasterModel organisationCentre = _organisationCentreMasterService.CreateOrganisationCentre(model);
        ...
        [HttpGet]
        [Produces(typeof(OrganisationCentreMasterModel))]
        public IActionResult GetOrganisationCentre(short organisationCentreMasterId)
        {
            try
            {
                OrganisationCentreMasterModel organisationCentreMasterModel = _organisationCentreMasterService.GetOrganisationCentre(organisationCentreMasterId);
                return IsNotNull(organisationCentreMasterModel) ? CreateOKResponse(organisationCentreMasterModel) : NotFound();
            }
            catch (CoditechException ex)
            {
                _coditechLogging.LogMessage(ex, CoditechLoggingEnum.Components.OrganisationCentre.ToString(), TraceLevel.Warning);
                return CreateInternalServerErrorResponse(new OrganisationCentreMasterModel { HasError = true, ErrorCode = ex.ErrorCode });
            }
            catch (Exception ex)
            {
                _coditechLogging.LogMessage(ex, CoditechLoggingEnum.Components.OrganisationCentre.ToString(), TraceLevel.Error);
                return CreateInternalServerErrorResponse(new OrganisationCentreMasterModel { HasError = true, ErrorMessage = ex.Message });
            }
        }
        ...
        [Produces(typeof(OrganisationCentreResponse))]
        public IActionResult UpdateOrganisationCentre([FromBody] OrganisationCentreMasterModel model)
        {
            try
            {
