...
namespace Coditech.API.Client
{
    public class HospitalDoctorsClient : BaseClient, IHospitalDoctorsClient
    {
        ...
        public virtual HospitalDoctorScheduleResponse GetHospitalDoctorSchedule(int hospitalDoctorId)
        {
            return Task.Run(async () => await GetHospitalDoctorScheduleAsync(hospitalDoctorId, CancellationToken.None)).GetAwaiter().GetResult();
        }

        public virtual async Task<HospitalDoctorScheduleResponse> GetHospitalDoctorScheduleAsync(int hospitalDoctorId, CancellationToken cancellationToken)
        {
            if (hospitalDoctorId <= 0)
                throw new System.ArgumentNullException("hospitalDoctorId");

            string endpoint = hospitalDoctorsEndpoint.GetHospitalDoctorScheduleAsync(hospitalDoctorId);
            HttpResponseMessage response = null;
            var disposeResponse = true;
            try
            {
                ...
                if (status_ == 200)
                {
                    var objectResponse = await ReadObjectResponseAsync<HospitalDoctorScheduleResponse>(response, headers_, cancellationToken).ConfigureAwait(false);
                    if (objectResponse.Object == null)
                    {
                        throw new CoditechException(objectResponse.Object.ErrorCode, objectResponse.Object.ErrorMessage);
                    }
                ...
                if (status_ == 204)
                {
                    return new HospitalDoctorScheduleResponse();
                }
                else
                {
                    string responseData = response.Content == null ? null : await response.Content.ReadAsStringAsync().ConfigureAwait(false);
                    HospitalDoctorScheduleResponse typedBody = JsonConvert.DeserializeObject<HospitalDoctorScheduleResponse>(responseData);
                    UpdateApiStatus(typedBody, status, response);
                    throw new CoditechException(status.ErrorCode, status.ErrorMessage, status.StatusCode);
                }
            }
        ...
        public virtual HospitalDoctorScheduleResponse UpdateHospitalDoctorSchedule(HospitalDoctorScheduleModel body)
        {
            return Task.Run(async () => await UpdateHospitalDoctorScheduleAsync(body, CancellationToken.None)).GetAwaiter().GetResult();
        }

        public virtual async Task<HospitalDoctorScheduleResponse> UpdateHospitalDoctorScheduleAsync(HospitalDoctorScheduleModel body, CancellationToken cancellationToken)
        {
            string endpoint = hospitalDoctorsEndpoint.UpdateHospitalDoctorScheduleAsync();
            HttpResponseMessage response = null;
            var disposeResponse = true;
            try
            {
