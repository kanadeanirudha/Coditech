var OrganisationCentrewiseBuildingMaster = {
    Initialize: function () {
        OrganisationCentrewiseBuildingMaster.constructor();
    },

    constructor: function () {
    },

    GetOrganisationCentrewiseBuildingMasterByCentreCode: function () {
        $('#DataTables_SearchById').val("")
        if ($("#SelectedCentreCode").val() == "") {
            CoditechNotification.DisplayNotificationMessage("Please select centre.", "error");
        }
        else {
            CoditechDataTable.LoadList("OrganisationCentrewiseBuildingMaster", "List");
        }
    },
}
